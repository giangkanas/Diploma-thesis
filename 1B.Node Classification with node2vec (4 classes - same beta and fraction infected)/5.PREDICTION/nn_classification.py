...
def HyperBand(filename):
    ...
    """CollegeMsg =>best batch size = 512 """
    """Facebook =>best batch size = 64 """
    """WikiVotes =>best batch size = 64 """
...
def baseline_model2():
    """ ΜΟΝΤΕΛΟ ΠΟΥ ΠΡΟΕΚΥΨΕ ΑΠΟ HYPERBAND """
    model = Sequential([
        Dense(232, input_dim = 256, activation='relu'),
...
def baseline_model3():
    """ ΜΟΝΤΕΛΟ ΠΟΥ ΠΡΟΕΚΥΨΕ ΑΠΟ HYPERBAND """
    model = Sequential([
        Dense(248, input_dim = 256, activation='relu'),
...
def crossValidation(filename):
    ...
    """Facebook =>best batch size = 64 """
    """WikiVotes =>best batch size = 64 """
    estimator = KerasClassifier(build_fn=baseline_model2, epochs=50, batch_size=64, verbose=1 )
