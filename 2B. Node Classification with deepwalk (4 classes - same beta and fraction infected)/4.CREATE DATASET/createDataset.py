...
def export_Dataset(filename):
    ...
    """ Τρέχοντας για ολα τα embeddings προεκυψε ότι:
        CollegeMsg => dic[5] =>emd_(dim=16_numWalks=25_walkLength=100).csv
        Facebook => dic[0] => emd_(dim=128_numWalks=20_walkLength=100).csv
        Wiki-Votes => dic[7] => emd_(dim=256_numWalks=15_walkLength=40).csv"""
    ...
    if filename=="CollegeMsg":
        ...
        dataset[16] = labels[0] #Στην τελευταια στηλη βαζω τα labels
...
if "__main__":
    ...
    for filename in filenames:
        ...
        dataset = export_Dataset(filenames[1])
        break
