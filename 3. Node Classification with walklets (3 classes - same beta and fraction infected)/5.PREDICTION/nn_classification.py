...
def baseline_model1():
    ...
    model = Sequential([
        Dense(208, input_dim = 32, activation='relu'),
...
def baseline_model2():
    """ ΜΟΝΤΕΛΟ ΠΟΥ ΠΡΟΕΚΥΨΕ ΑΠΟ HYPERBAND """
    model = Sequential([
        Dense(472, input_dim = 128, activation='relu'),
...
def crossValidation(filename):
    ...
    """Facebook =>best batch size = 512 """
    """WikiVotes =>best batch size = 512 """
    estimator = KerasClassifier(build_fn=baseline_model3, epochs=50, batch_size=512, verbose=1 )
...
def predictInfluence(filename):
    ...
    history = model.fit(x_train,y_train,epochs=50,batch_size=512,validation_data=(x_test, y_test),callbacks= callback)
