...
def export_Dataset(filename):
    ...
    """ Τρέχοντας για ολα τα embeddings προεκυψε ότι:
        CollegeMsg => dic[4] =>emd_(dim=256_numWalks=15_walkLength=40).csv
        Facebook => dic[1] => emd_(dim=128_numWalks=20_walkLength=120).csv
        Wiki-Votes => dic[9] => emd_(dim=64_numWalks=25_walkLength=100).csv"""
    
    if filename=="CollegeMsg":
        node_embeddings , labels , count = data(filename,4)
        result = crossValidation(filename,4)
        print(result)
        ...
        node_embeddings , labels , count = data(filename,9)
        result = crossValidation(filename,9)
        print(result)
        dataset = node_embeddings
