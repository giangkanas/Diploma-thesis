...
def build_model(hp):
    ...
    hp_units1 = hp.Int('units1', min_value=8, max_value=256, step=8)
    ...
    model.compile(
        optimizer=opt.Adam(learning_rate=hp_learning_rate),
...
def HyperBand(filename):
    ...
    """WikiVotes =>best batch size = 128 """
    
def baseline_model1():
    """ ΜΟΝΤΕΛΟ ΠΟΥ ΠΡΟΕΚΥΨΕ ΑΠΟ HYPERBAND """
    model = Sequential([
        Dense(256, input_dim = 16, activation='relu'),
...
def baseline_model2():
    """ ΜΟΝΤΕΛΟ ΠΟΥ ΠΡΟΕΚΥΨΕ ΑΠΟ HYPERBAND """
    model = Sequential([
        Dense(256, input_dim = 256, activation='relu'),
...
def baseline_model3():
    """ ΜΟΝΤΕΛΟ ΠΟΥ ΠΡΟΕΚΥΨΕ ΑΠΟ HYPERBAND """
    model = Sequential([
        Dense(248, input_dim = 128, activation='relu'),
...
def crossValidation(filename):
    ...
    """Facebook =>best batch size = 64 """
    """WikiVotes =>best batch size = 128 """
    estimator = KerasClassifier(build_fn=baseline_model3, epochs=50, batch_size=128, verbose=1 )
